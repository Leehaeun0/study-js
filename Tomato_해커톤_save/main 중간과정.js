...
const render = () => {
  ...
  _todos.forEach(todo => {
    html += `<li id="${todo.id}" class="${todo.color} ing">
...
const getTodayPersent = () => {
  ...
  $todayPercent.textContent = `${Math.round(((nowHour * 60 + nowMin) / (goalHour * 60 + goalMin)) * 100)}%`;
  // console.log('평균계산', (nowHour * 60 + nowMin), (goalHour * 60 + goalMin));
};
